...
docker run --rm --name $NAME -it -d -p $PORT:40041 -v $MOLECULE_ENV_HOME/../molecule-benchmarks/:/home -v $MOLECULE_ENV_HOME/ipc-dag-src/:/env -w /env --entrypoint=/env/scripts/local_baseline_callee.sh ddnirvana/molecule-js-env:v3-node14.16.0 $1
