...
function parse_logs(){
	...
	cat $LOG_DIR/$1-callee_logs.txt |  sed "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" |  grep "exe (handler)" | awk '{print $5}'
	echo "Caller results:"
	#cat $LOG_DIR/$1-caller_logs.txt | sed "s,\x1B\[[0-9;]*[a-zA-Z],,g" | grep "callee comm" | awk '{print $9}'
	cat $LOG_DIR/$1-caller_logs.txt | sed "s/\x1B\[([0-9]{1,3}(;[0-9]{1,2})?)?[mGK]//g" | grep "callee comm" | awk '{print $9}'
	echo ""
}
